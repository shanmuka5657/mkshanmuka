...
const FinancialRiskOutputSchema = z.object({
  ...
  foirAnalysis: z.object({
      ...
      explanation: z.string().describe('A detailed, point-wise explanation of the FOIR and its impact on disposable income and loan eligibility.')
...
const prompt = ai.definePrompt({
  ...
  prompt: `You are a financial risk analyst. Your task is to assess the user's overall financial risk based on their structured credit data, estimated income, and fixed obligations. Your analysis must be detailed, non-generic, and provide real-world context in a point-wise format. Do NOT use raw text.
...
4.  **FOIR Analysis (foirAnalysis):**
    ...
    *   In 'explanation', provide a point-wise explanation. Explain that FOIR provides a more holistic view of financial obligations. Discuss how a high FOIR (typically above 50-55%) can severely limit disposable income and make it difficult to secure new loans, as lenders use this metric to assess repayment capacity.
...
7.  **Overall Outlook (overallOutlook):**
    *   Provide a final, forward-looking summary in a point-wise format. Synthesize all the above points into a clear outlook (e.g., "Given your high FOIR and reliance on credit card debt... an unexpected expense could be difficult to manage. The immediate priority should be to reduce the balance on your high-utilization credit cards.").
